...
class MetaImageNet(ImageNet):
    ...
    def __init__(self,
                 ...
                 test_transform=None,
                 fix_seed=True):
        super(MetaImageNet, self).__init__(args, partition=partition, pretrain=False)
...
if __name__ == '__main__':
    ...
    args.n_shots = 1
    args.n_queries = 12
    args.data_root = json.load(open('config.json'))['miniImageNet']
    args.data_aug = True
    args.n_test_runs = 5
    args.n_aug_support_samples = 1
    imagenet = ImageNet(args, partition='train')
    print(len(imagenet))
    print(imagenet.__getitem__(500)[0].shape)

    metaimagenet = MetaImageNet(args, partition='val')
